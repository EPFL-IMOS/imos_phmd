...
BEARING_FAULT_TYPE = {
    ...
    (1, 5): [
        "Inner race",
        "Outer race",
    ],  # ['Inner race', 'Outer race'] Fix from ['Inner race', 'Cage'] to ['Inner race', 'Outer race'] as stated in table 4 of   Fault diagnosis of rolling bearing failures using a multi-stage e-CNN-GRU-SAM network.
    (2, 1): ["Inner race"],
    (2, 2): ["Outer race"],
    (2, 3): ["Cage"],
    (2, 4): ["Outer race"],
    ...
    (3, 2): [
        ...
        "Inner race",
        "Cage",
    ],  # Table 4 of the same paper as above also have smth like 'ball' as failure.
    (3, 3): ["Inner race"],
    (3, 4): ["Inner race"],
    (3, 5): ["Outer race"],
}
...
def filter_files(dirs, files, filters: dict = None):
    ...
    for bearing in dirs:
        ...
        if filters is not None:
            ...
            if "Fault_element" in filters and not any(
                fe in filters["Fault_element"] for fe in fault_element
            ):
                continue
    ...
    with tqdm.tqdm(total=len(files), desc="Reading dataset") as pbar:
        ...
        for bearing in dirs:
            ...
            bearing_files = sorted(
                [f for f in files if bearing in f],
...
def read_dataset(file_path: str, task: dict = None, filters: dict = None):
    ...
    if os.path.isdir(file_path):
        ...
        with zipfile.ZipFile(file_path) as z:
            ...
            datasets = read_files(
                file_path, dirs, files, z=z, filters=filters, task=task
            )
